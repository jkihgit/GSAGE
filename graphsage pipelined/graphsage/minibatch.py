...
class TFBatching(object):
    ...
    def __init__(self, init_type, inputs):
        ...
        ranktemplate = [0,0]
        ranktemplate[0] = np.zeros((FLAGS.batch_size, FLAGS.samples_2, 1), dtype=np.float32)
        ranktemplate[1] = np.zeros((FLAGS.batch_size*FLAGS.samples_2, FLAGS.samples_1, 1), dtype=np.float32)
        valranktemplate = [0,0]
        valranktemplate[0] = np.zeros((FLAGS.validate_batch_size, FLAGS.samples_2, 1), dtype=np.float32)
        valranktemplate[1] = np.zeros((FLAGS.validate_batch_size*FLAGS.samples_2, FLAGS.samples_1, 1), dtype=np.float32)
        for template in ranktemplate:
            template = fillTemplate(template)
        for template in valranktemplate:
            template = fillTemplate(template)
        self.ranktemplate = ranktemplate
        self.valranktemplate = valranktemplate
    ...
    def init_old(self, inputs):
        ...
        # self.train_nodes = self.train_nodes[:8192]
    ...
    def init(self, prefetch_depth):
        ...
        def samplingLayer(adj, inputs, name):
            ids, num_samples, layer, ranktemplate = inputs
            ...
            template = tf.slice(ranktemplate[layer], [0,0,0], tf.shape(randpicks), name='sampSlice_'+name)
            ...
            return adj_lists
        def fetchBatchFeatures(nodeIDs, labels, adj, batch_size, ranktemplate):
            ...
            fvs0 = tf.reshape(fvs0, (batch_size, 1, -1), name='reshape_0')

            adj1 = samplingLayer(adj, (nodeIDs, FLAGS.samples_2, 1, ranktemplate), name='1')
            fvs1 = tf.nn.embedding_lookup(self.features, adj1, name='aggemb_1')
            fvs1 = tf.reshape(fvs1, (batch_size, FLAGS.samples_2, -1), name='reshape_1')

            adj2 = samplingLayer(adj, (adj1, FLAGS.samples_1, 2, ranktemplate), name='2')
            fvs2 = tf.nn.embedding_lookup(self.features, adj2, name='aggemb_2')
            fvs2 = tf.reshape(fvs2, (batch_size, FLAGS.samples_2, FLAGS.samples_1, -1), name='reshape_2')
            fvs2 = tf.reduce_mean(fvs2, axis=2, name='initial_reduce')
            # fvs0: BSx1xFV
            # fvs1: BSxS2xFV
        ...
        def dsmap(nodeIDs, labels):
            ...
            return fetchBatchFeatures(nodeIDs, labels, self.adj, self.batch_size, self.ranktemplate)
        def testdsmap(nodeIDs, labels):
            # with tf.device('device:XLA_CPU:0'):
            return fetchBatchFeatures(nodeIDs, labels, self.test_adj, self.validation_batch_size, self.valranktemplate)
        ...
        self.ds = self.ds.interleave(lambda x,y: tf.data.Dataset.from_tensor_slices((dsmap(x, y))), cycle_length=2, block_length=2)
        self.ds = self.ds.batch(self.batch_size, drop_remainder=True)
