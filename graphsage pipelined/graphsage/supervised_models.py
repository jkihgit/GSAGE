...
class SupervisedGraphsage(models.SampleAndAggregate):
    ...
    def __init__(self, num_classes,
            legacy_var_list, features, degrees,
        ...
        self.train_model = self.build(legacy_var_list["batch_size"], legacy_var_list["gpu_count"])
        if (not FLAGS.minimini) and (not FLAGS.noval):
            self.val_model = self.build(legacy_var_list["val_batch_size"], legacy_var_list["gpu_count"])

    def build(self, batch_size, gpu_count):
        num_feats = 1 + FLAGS.samples_2 + FLAGS.samples_2
        fvsize = self.minibatch.fvsize
        batch = tf.keras.Input(shape=(num_feats, fvsize), batch_size=batch_size*gpu_count, dtype=tf.float32)
        outputs1 = self.aggregate(batch, self.dims, batch_size, concat=self.concat, model_size=self.model_size, name="")
    ...
    def minimini(self, epochs=1):
        ...
        with graph.as_default():
            ...
            x, y = self.minibatch.entire_training_set().take(1)
            callbacks = []
            callbacks += [TimeMeasurementsCallback()]
            history = self.train_model.fit(x=x, epochs=epochs, callbacks=callbacks)
    ...
    def train(self, verbose=2):
        ...
        with graph.as_default():
            ...
            callbacks += [TimeMeasurementsCallback()]
            if FLAGS.patience > 0:
                _, ground_truth = self.minibatch.raw_validation_set()
                ds = self.minibatch.entire_validation_set()
                callbacks += [EarlyTerminationCallback(FLAGS.patience, self.val_model, ds, ground_truth)]
            # histograms
            if FLAGS.tensorboard:
            ...
            ds = self.minibatch.entire_training_set()
            vds = None
            validation_freq = None
            if (not FLAGS.noval):
                vds = self.minibatch.entire_testing_set()
                validation_freq = FLAGS.validate_iter
            history = self.train_model.fit(ds, epochs=FLAGS.epochs, validation_data=vds, validation_freq=validation_freq, callbacks=callbacks, verbose=verbose)
    ...
    def validate(self, verbose=0):
        ...
        preds = self.val_model.predict(self.minibatch.entire_validation_set(), callbacks=callbacks, verbose=verbose)
        f1s = calc_f1(ground_truth, preds)
        print ('F1 micro', f1s[0], 'F1 macro', f1s[1])
        return s1, f1s
...
class EarlyTerminationCallback(tf.keras.callbacks.Callback):
    def __init__(self, patience, val_model, ds, val_y):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        preds = self.val_model.predict(self.ds, verbose=0)
