...
class SampleAndAggregate(GeneralizedModel):
    ...
    def aggregate(self, input_features, dims, batch_size,
        ...
        """ At each layer, aggregate hidden representations of neighbors to compute the hidden representations 
            at next layer.
        Args:
            input_features: the input features arranged in a 21x(FVLEN) matrix, starting with the input node's FV, immediate neighbor nodes' FV, and reduced 2nd order neighbor nodes' FV
        ...
        for layer in range(len(num_samples)):
            if new_agg:
                ...
                if layer == len(num_samples) - 1:
                    act = lambda x : x
                aggregator = self.aggregator_cls(dim_mult*dims[layer], dims[layer+1], name=name+str(layer), concat=concat, model_size=model_size, act=act)
