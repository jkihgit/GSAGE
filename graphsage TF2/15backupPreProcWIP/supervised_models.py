...
class SupervisedGraphsage(models.SampleAndAggregate):
    ...
    def __init__(self, num_classes,
        ...
        if (not FLAGS.minimini) and (not FLAGS.noval):
            # do validation preproc or sth idk ill get around to it later
            self.val_model = self.build(legacy_var_list["val_batch_size"], legacy_var_list["gpu_count"])

    def build(self, batch_size, gpu_count):
        # shape is feat vec for input node, its neighbors, and their neighbors
        # in what order tho
        # and also this is an irregularly shaped tensor
        # i can flatten it, use that, and reshape it after receiving it

        hidden = tf.keras.Input(shape=(), batch_size=batch_size*gpu_count, dtype=tf.float32)
        support_sizes1 = self.getSupportSizes(self.layer_infos)
        # reshape hidden back to the irregular shaped tensor
        num_samples = [layer_info.num_samples for layer_info in self.layer_infos]
        outputs1 = self.aggregate(hidden, self.dims, num_samples,
