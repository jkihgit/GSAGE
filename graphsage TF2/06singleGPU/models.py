...
class SampleAndAggregate(GeneralizedModel):
    ...
    def sample(self, inputs, layer_infos, batch_size=None):
        ...
        for k in range(len(layer_infos)):
            ...
            node = tf.keras.layers.Lambda(sampler, name='sampler'+str(k))((samples[k], layer_infos[t].num_samples))
            samples.append(node)
        ...
        for layer in range(len(num_samples)):
            if new_agg:
                ...
                if layer == len(num_samples) - 1:
                    act = lambda x : x
                aggregator = self.aggregator_cls(dim_mult*dims[layer], dims[layer+1], batch_size, name=name+str(layer), concat=concat, model_size=model_size)
