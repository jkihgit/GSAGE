...
class SupervisedGraphsage(models.SampleAndAggregate):
    ...
    def __init__(self, num_classes,
            ...
            model_size="small", sigmoid_loss=False, identity_dim=0, late_compile=False,
                **kwargs):
        '''
        Args:
        ...
        self.gpu_count = legacy_var_list["gpu_count"]
        self.legacy_var_list = legacy_var_list
        self.layer_infos = layer_infos
        self.late_compile = late_compile

        self.build()


    def build(self):
        batch = tf.keras.Input(shape=(), batch_size=self.batch_size*self.gpu_count, dtype=tf.int32)
        ...
        # TF1 copy
        self.opt = tf.keras.optimizers.Adam(learning_rate=0.01, epsilon=1e-8, clipvalue=5.0)
        ...
        if not self.late_compile:
            self.model.compile(optimizer=self.opt, loss=self.loss, metrics=['binary_accuracy', 'categorical_accuracy'])
        self.model.summary()

    def perform_late_compile(self):
        assert self.late_compile
        self.model.compile(optimizer=self.opt, loss=self.loss, metrics=['binary_accuracy', 'categorical_accuracy'])


    def minimum_viable(self, x, y, batch_size):
        return self.model.fit(x=x, y=y, epochs=1, batch_size=batch_size)
    ...
    def train(self, trnds, tstds, vds, verbose=2):
        ...
        history = self.model.fit(trnds, epochs=10, validation_data=tstds, validation_freq=FLAGS.validate_iter, callbacks=callbacks, verbose=verbose)
