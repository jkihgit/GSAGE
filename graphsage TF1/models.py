...
class SampleAndAggregate(GeneralizedModel):
    ...
    def __init__(self, placeholders, features, adj, degrees,
        ...
        self.adj_info = adj
        if identity_dim > 0:
           self.embeds = tf.get_variable("node_embeddings", [adj.get_shape().as_list()[0], identity_dim], name="aggembed")
        else:
           self.embeds = None
        if features is None: 
            if identity_dim == 0:
    ...
    ## @nvtx_tf.ops.trace(message='Sampling Super Block', domain_name='Forward', grad_domain_name='Gradient', enabled=FLAGS.enable_nvtx, trainable=True)
    def sample(self, inputs, layer_infos, batch_size=None):
        ...
        samples: a 2D list of adjacent nodes in float by batch size, similar to how the adj mat compression works
                fed directly to aggregate() by supervised_models.build()
        """
        
        # lets see if this is ever not none
        ...
        for k in range(len(layer_infos)):
            ...
            #sampler, nvtx_ctx = nvtx_tf.ops.start(sampler, message='Sampling '+str(k), domain_name='Forward', grad_domain_name='Gradient', enabled=FLAGS.enable_nvtx)
            node = sampler((samples[k], layer_infos[t].num_samples, t))
            #node = nvtx_tf.ops.end(node, nvtx_ctx)
            samples.append(tf.reshape(node, [support_size * batch_size,]))
            support_sizes.append(support_size)
        return samples, support_sizes


    ## @nvtx_tf.ops.trace(message='Aggregate Super Block', domain_name='Forward', grad_domain_name='Gradient', enabled=FLAGS.enable_nvtx)
    def aggregate(self, samples, input_features, dims, num_samples, support_sizes, batch_size=None,
            aggregators=None, name=None, concat=False, model_size="small", bypassFlags=[]):
        ...
        Args:
            ...
            num_samples: list of number of samples for each layer. e.g. 25, 10 by default
            support_sizes: the number of nodes to gather information from for each layer.
                            i.e. the numer of hops to make
            batch_size: the number of inputs (different for batch inputs and negative samples).
        Returns:
            The hidden representation at the final layer for all nodes in batch
        """
        # dims = [inputDims, dim1, dim2]
        ...
        if do_fetch:
            # returns feature vectors for each neighborhood
            # [[batchsize x fetveclen], [batchsize*samplingRate2 x fetveclen], [batchsize*samplingRate2*samplingRate1 x fetveclen]]
            #input_features, nvtx_ctx = nvtx_tf.ops.start(input_features, message='Aggregation Feature Fetch', domain_name='Forward', grad_domain_name='Gradient', enabled=FLAGS.enable_nvtx)
            if FLAGS.lookup_fix:
                with tf.device('/gpu:0'):        
                    hidden = [tf.nn.embedding_lookup(input_features, tf.cast(node_samples, tf.int32), name="aggembed") for node_samples in samples]
            else:
                hidden = [tf.nn.embedding_lookup(input_features, node_samples, name="aggembed") for node_samples in samples]
            #hidden = nvtx_tf.ops.end(hidden, nvtx_ctx)
        else:
            raise NotImplementedError
            # problem: this is a pretty big workload
            # abort abort
            hidden = [tf.cast(x, tf.float32) for x in samples]


            # input: id of nodes in each neighborhood
            # output: fetvec for each
            # 1) cast to float
            # 2) tile by fetveclen
            # 3) reshape to (-1, fetveclen)
            # not sure what this error is, but it might be from trying to use pylist mapping in tf/np
            hidden = [tf.tile(x, [1, input_features]) for x in hidden]
            # hidden = [tf.reshape(x, [self.batch_size-1]) for x in hidden]
        if do_nn:
            ...
            for layer in range(len(num_samples)):
                ...
                for hop in range(len(num_samples) - layer):
                    ...
                    neigh_dims = [batch_size * support_sizes[hop], 
                                  ...
                                  dim_mult*dims[layer]]
                    #hidden[hop], nvtx_ctx = nvtx_tf.ops.start(hidden[hop], message='Aggregation Layer '+str(layer)+' hop '+str(hop), domain_name='Forward', grad_domain_name='Gradient', enabled=FLAGS.enable_nvtx)
                    h = aggregator((hidden[hop], tf.reshape(hidden[hop + 1], neigh_dims)))
                    #h = nvtx_tf.ops.end(h, nvtx_ctx)
                    # h = tf.Print(h, [tf.shape(h)], 'asdfasdfasdfasdfasdf')
                    next_hidden.append(h)
                hidden = next_hidden
            # hidden[0] = tf.Print(hidden[0], [tf.shape(hidden[0])], 'xxxxxxxxxxxxxxxddd')
            return hidden[0], aggregators
        raise NotImplementedError
        return hiddn, None

    # not used in normal op
    def _build(self):
        labels = tf.reshape(
        ...
        assert len(self.inputs1) == len(self.inputs2)
        # perform "convolution"
    ...
    # no impact on accuracy with the two building functions commented out
    # i guess its only used by unsupervised GraphSAGE
