...
class TFBatching(object):
    ...
    def __init__(self, G, id2idx, 
            placeholders, label_map, num_classes, 
            batch_size=100, max_degree=25, prefetch_depth=1, validation_batch_size=-1, threads=8,
            **kwargs):
        ...
        # self.train_nodes, self.train_labels, self.train_batch_sizes = self.processNodes(self.train_nodes, self.batch_size)
        # self.test_nodes, self.test_labels, self.test_batch_sizes = self.processNodes(self.test_nodes, self.batch_size)
        # self.val_nodes, self.val_labels, self.val_batch_sizes = self.processNodes(self.val_nodes, self.validation_batch_size)
        ...
        self.dsTest = self.dsTest.batch(self.validation_batch_size)
        ...
        self.dsVal = self.dsVal.batch(self.validation_batch_size)
        self.dsVal = self.dsVal.prefetch(buffer_size=prefetch_depth)
        
        self.iter = tf.data.Iterator.from_structure(self.ds.output_types, self.ds.output_shapes)
        self.dsInit = self.iter.make_initializer(self.ds)
        self.dsInitTest = self.iter.make_initializer(self.dsTest)
        self.dsInitVal = self.iter.make_initializer(self.dsVal)



    def processNodes(self, nodes): #, batch_size):
        labels = np.array([self._make_label_vec(n) for n in nodes], dtype=np.float32)
        nodes = [self.id2idx[n] for n in nodes]
        nodes = np.array(nodes, dtype=np.int32)
        # batch_sizes = self.batches(nodes, batch_size)
        return nodes, labels #, batch_sizes
    ...
    # check if there's any more training data
    def end(self, batch_num):
        return batch_num * self.batch_size >= len(self.train_nodes)

    def get_batch_size(self, batchnum):
        start_idx = batchnum * self.batch_size
        end_idx = min(start_idx + self.batch_size, len(self.train_nodes))        
        return end_idx - start_idx

    def get_batch_size_test(self, batchnum):
        start_idx = batchnum * self.batch_size
        end_idx = min(start_idx + self.batch_size, len(self.test_nodes))        
        return end_idx - start_idx

    def get_batch_size_val(self, batchnum):
        start_idx = batchnum * self.batch_size
        end_idx = min(start_idx + self.batch_size, len(self.val_nodes))        
        return end_idx - start_idx

    def get_labels(self, batchnum):
        start_idx = batchnum * self.batch_size
        end_idx = min(start_idx + self.batch_size, len(self.train_nodes))  
        return self.train_labels[start_idx: end_idx]

    def get_labels_test(self, batchnum):
        start_idx = batchnum * self.batch_size
        end_idx = min(start_idx + self.batch_size, len(self.test_nodes))  
        return self.test_labels[start_idx: end_idx]

    def get_labels_val(self, batchnum):
        start_idx = batchnum * self.batch_size
        end_idx = min(start_idx + self.batch_size, len(self.val_nodes))  
        return self.val_labels[start_idx: end_idx]

    # init
    # returns list of batch sizes
    def batches(self, nodes, batch_size):
        dropped_size = len(nodes) // batch_size
        # make a filled dropped_size long array of batch_size
        # e.g. [1]*2 = [1,1]
        out = [batch_size]*dropped_size
        # wouldnt this cause tolerance issues?
        # not at our dataset sizes
        if (float(len(nodes)) / batch_size) > dropped_size:
            out.append(len(nodes)%batch_size)
        return out
